...
const ListInvoices = () => {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        const response = await axios.get('endearing-ambition-production.up.railway.app/api/invoices', {
          headers: {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this invoice?')) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`endearing-ambition-production.up.railway.app/api/invoices/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
